// Ensure proxyFetch is loaded to patch globalThis.fetch — without this,
// token exchange requests ignore proxyOptions and go direct (CF-blocked).
...
async function completeXaiManualCode(code, state, proxyPoolId = null) {
  ...
  const proxyConfig = proxyPoolId ? await resolveConnectionProxyConfig({ proxyPoolId }) : null;
  ...
  try {
    ...
    if (action === "exchange") {
      ...
      if (code && code.startsWith("eyJ") && code.includes(".")) {
        ...
        // Store agent identity key material for AgentAssertion auth
        if (directPayload.agent_runtime_id) providerSpecificData.agentRuntimeId = directPayload.agent_runtime_id;
        if (directPayload.agent_private_key) providerSpecificData.agentPrivateKey = directPayload.agent_private_key;
      ...
      const tokenData = await exchangeTokens(provider, code, redirectUri, codeVerifier, state, meta, proxyOptions);
